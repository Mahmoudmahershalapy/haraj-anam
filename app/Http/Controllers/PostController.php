...
if($request->file('file_name')){
        ...
        if(in_array($extention, $imageExtensions)){
...
return redirect('/add');
           ...
           $image->save($destnation_path.'/'.$filename);
            ...
            if ((int) number_format($image->getSize() / 1048576,2) >  (int)\App\SiteSetting::where('name','max_upload_size')->first()->value)
            {
    ...
    $videoCount++;
         ...
         //   return (int)\App\SiteSetting::where('name','max_upload_posts_images')->first()->value;
